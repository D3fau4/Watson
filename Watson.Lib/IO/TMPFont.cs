...
public class TMPFont
{
    ...
    public TMPFont(string FontBundle, Assembly assembly)
    {
        ...
        {
            var deserialized =
                MonoDeserializer.GetMonoBaseField(m_Assets.AM, m_Assets.Assets, Asset, m_DLL.AssemblyFolder);
            var asset = deserialized.Get("m_fontInfo");
            if (asset != null)
                // Almacenar el nombre de asset que contiene la fuente.
                m_FontNames.Add(Asset.index,
                    Tuple.Create(deserialized.Get("m_Name").GetValue().AsString(), deserialized, Asset,
                        m_Assets.Assets));
        }
        ...
        foreach (var Texture in m_Assets.GetAssetsOfType(AssetClassID.Texture2D))
        {
            ...
            m_FontTextures.Add(Texture.index,
